...
const dividend = artifacts.require("TOKENDividendTracker");
...
contract("Token Gas Reduce", (accounts) => {
  ...
  before(async function () {
      ...
      iterableMappingInstance = await iterableMapping.new();
      await dividend.link(iterableMappingInstance);
      dividendInstance = await dividend.new();
   ...
   describe("ShiborgInuEther Token", () => {
      it("admin token transfer", async function () {
        ...
        console.log("Hash",String(await pancakeFactoryInstance.INIT_CODE_PAIR_HASH()) );
        await WETHinstance.approve(user1,amount, {from: owner});

        await dividendInstance.initialize(
            owner,
            "1000000"
        )
      }); 
